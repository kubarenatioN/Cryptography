...
// hex 2 chars to binary
const hexToBin = (hex) =>
	("00000000" + parseInt(hex, 16).toString(2)).substr(-8);

const decToBin = (dec) => ("0000" + parseInt(dec, 10).toString(2)).substr(-4);

const binToHex = (str) => parseInt(str, 2).toString(16).toUpperCase();

// convert hex to binary dividing into chunks
const bin = (key) =>
	chunkString(key, 2)
...
// create keys list for each round
const keySchedule = (key) => {
	let subkeys = [];

	// first key permutation
	let perm = PC1.map((index) => key[index - 1]).join("");
	let left = perm.substr(0, perm.length / 2);
	let right = perm.substr(perm.length / 2);
	let prevLeft = left,
		prevRight = right;
	NUM_OF_LEFT_SHIFTS.forEach((shift, i) => {
		left = shiftString(prevLeft, shift);
		right = shiftString(prevRight, shift);
		prevLeft = left;
		prevRight = right;
		let pair = left + right;
...
const encode = (text, key) => {
	const binary = bin(text);
	// console.log(binary);
	return des(binary, key, keySchedule(key));
};
const decode = (cipher, key) => {
	const binary = bin(cipher);
	return des(binary, key, keySchedule(key).reverse());
};

const message = "NIKITAVALERIEVICH";
// const message = '0123456789ABCDEF'
let key = bin("133457799BBCDFF1"); // hex

let enc = [];
let messageBlocks = chunkString(message, 8);
messageBlocks = messageBlocks.map((block) => {
	return block.padEnd(8, "X");
});
console.log(messageBlocks);
messageBlocks = messageBlocks.map((block) => {
	return block.hexEncode();
});
// console.log(messageBlocks);

messageBlocks.forEach((block) => {
	// console.log('blocks', block);
	const e = encode(block, key);
	// console.log('block encoding', e);
	enc.push(e);
});
// let enc = encode(message, key);
// let dec = decode(enc, key)

console.log("Message:", messageBlocks.join(" ")); // => 0123456789ABCDEF
console.log("Ecnoding:", enc); // => 85E813540F0AB405

let dec = "";
const cipherBlocks = chunkString(enc.join(""), 16);
cipherBlocks.forEach((block) => {
	// console.log('block decode', block);
	dec += decode(block, key);
});
console.log("Deconding:", dec); // => 0123456789ABCDEF
console.log("Human Decoding:", dec.hexDecode());
